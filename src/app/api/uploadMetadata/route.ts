...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (
      !metadataToPin ||
      typeof metadataToPin !== 'object' ||
      !metadataToPin.name ||
      !metadataToPin.prompt_text
    ) {
      console.error('[API /api/uploadMetadata] Invalid or incomplete metadata received.');
      return NextResponse.json(
        {
          success: false,
          error: 'Invalid or incomplete metadata. Must include name and prompt_text.',
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      '[API /api/uploadMetadata] Sending JSON to Pinata. Metadata Name:',
      pinataData.pinataMetadata.name
    );
    ...
    console.error(
      '[API /api/uploadMetadata] Error during IPFS JSON upload:',
