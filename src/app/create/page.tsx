...
export default function CreatePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.title || !formData.promptText || !formData.platform) {
      toast({
        ...
        description: 'Title, Platform, and Prompt Text are required.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (formData.includeMedia && inputMediaFiles.length > 0) {
        ...
        for (const mediaWrapper of inputMediaFiles) {
          ...
          if (!res.ok || !result.success)
            throw new Error(result.error || 'Input media file upload failed');
      ...
      if (includeOutputSample && outputSampleFiles.length > 0) {
        ...
        for (const mediaWrapper of outputSampleFiles) {
          ...
          const result = await res.json();
          if (!res.ok || !result.success)
            throw new Error(result.error || 'Output sample file upload failed');
      ...
      const metadataToUpload = {
        ...
        image:
          finalOutputSampleCIDs.length > 0
            ? `ipfs://${finalOutputSampleCIDs[0]}`
            : finalInputMediaCIDs.length > 0
              ? `ipfs://${finalInputMediaCIDs[0]}`
              : undefined,
      ...
      toast({
        ...
        description: `Token ID: ${result.tokenId}. Your NFT "${formData.title}" is live.`,
      });
      router.push(result.tokenId ? `/prompt/${result.tokenId}` : `/profile`);
    } catch (error: any) {
  ...
  return (
    <div className="container mx-auto max-w-3xl py-12">
      <h1 className="gradient-text mb-8 text-center text-3xl font-bold md:text-4xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <Input
                id="title"
                name="title"
                placeholder="Epic Space Battle Prompt"
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={handleChange}
                className="w-full rounded-md border border-input bg-card px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                required
              >
            ...
            <div className="space-y-2">
              ...
              <Textarea
                id="description"
                name="description"
                placeholder="A detailed prompt for generating..."
                value={formData.description}
                onChange={handleChange}
                className="min-h-[100px] border-input bg-card text-foreground placeholder:text-muted-foreground"
              />
            </div>
            {/* Prompt Text */}
            <div className="space-y-2">
              <Label htmlFor="promptText" className="text-foreground">
                Prompt Text *
              </Label>
              <Textarea
                id="promptText"
                name="promptText"
                placeholder="Your actual AI prompt..."
                value={formData.promptText}
                onChange={handleChange}
                className="min-h-[150px] border-input bg-card text-foreground placeholder:text-muted-foreground"
                required
              />
            </div>
            ...
            <div className="flex items-center space-x-2 pt-2">
              <Switch
                ...
                onCheckedChange={handleIncludeMediaSwitchChange}
              />
              <Label htmlFor="includeMedia" className="text-foreground">
                Include Input Media File(s)
              </Label>
            </div>
            {formData.includeMedia && (
              <div className="ml-2 space-y-4 border-l-2 border-border/50 pl-2">
                <Label htmlFor="inputMediaFiles" className="block pt-2 text-muted-foreground">
                  Upload Input Media
                </Label>
                <Input
                  id="inputMediaFiles"
                  ...
                  onChange={(e) => handleFileUploaderChange(e, 'input')}
                  className="border-input bg-card text-muted-foreground file:font-semibold file:text-primary"
                  multiple
                />
                {inputMediaFiles.length > 0 && (
                  <div className="text-sm text-muted-foreground">
                    Previews ({inputMediaFiles.length}):
                  </div>
                )}
                  ...
                  {inputMediaFiles.map((mf) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        onClick={() => removeFile(mf.id, 'input')}
                      >
                        <X size={16} />
                      </Button>
                    </div>
            ...
            <div className="flex items-center space-x-2 pt-4">
              <Switch
                id="includeOutputSample"
                checked={includeOutputSample}
                onCheckedChange={setIncludeOutputSample}
              />
              <Label htmlFor="includeOutputSample" className="text-foreground">
                Include Output Sample(s)
              </Label>
            </div>
            {includeOutputSample && (
              <div className="ml-2 space-y-4 border-l-2 border-border/50 pl-2">
                <Label htmlFor="outputSampleFiles" className="block pt-2 text-muted-foreground">
                  Upload Output Samples
                </Label>
                <Input
                  id="outputSampleFiles"
                  type="file"
                  accept="image/*,text/plain,.pdf"
                  onChange={(e) => handleFileUploaderChange(e, 'output')}
                  className="border-input bg-card text-muted-foreground file:font-semibold file:text-primary"
                  multiple
                />
                {outputSampleFiles.length > 0 && (
                  <div className="text-sm text-muted-foreground">
                    Previews ({outputSampleFiles.length}):
                  </div>
                )}
                <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {outputSampleFiles.map((mf) => (
                    ...
                    >
                      ...
                      <Button
                        ...
                        size="icon"
                        className="absolute right-1 top-1 h-6 w-6 opacity-0 group-hover:opacity-100"
                        onClick={() => removeFile(mf.id, 'output')}
                      >
                        <X size={16} />
                      </Button>
                    </div>
            ...
            <div className="pt-6">
              <Button
                type="submit"
                className="bg-gradient-button hover:bg-gradient-button-hover w-full py-3 text-lg font-semibold text-primary-foreground"
